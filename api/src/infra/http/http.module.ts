import { Module, forwardRef } from '@nestjs/common'
import { CryptographyModule } from 'src/infra/cryptography/cryptography.module'
import { DatabaseModule } from 'src/infra/database/database.module'
import { AuthModule } from '../auth/auth.module'
import { UpdatePlayerController } from './controllers/core/player/update-player.controller'
import { UpdatePlayerUseCase } from 'src/domain/core/application/use-cases/player/update-player'
import { CredentialsAuthenticateController } from './controllers/core/sessions/credentials-authenticate.controller'
import { CredentialsAuthenticateUseCase } from 'src/domain/core/application/use-cases/sessions/credentials-authenticate'
import { ValidateAccountUseCase } from 'src/domain/core/application/use-cases/accounts/validate-account'
import { MailModule } from '../mail/mail.module'
import { BullConfigModule } from '../schedules/bull/bull.module'
import { RegisterTabletopController } from './controllers/tabletop-finder/register-tabletop.controller'
import { RegisterTabletopLocationController } from './controllers/tabletop-finder/register-tabletop-location.controller'
import { RegisterTabletopUseCase } from 'src/domain/tabletop-finder/application/use-cases/register-tabletop'
import { RegisterTabletopLocationUseCase } from 'src/domain/tabletop-finder/application/use-cases/register-tabletop-location'
import { FetchNearbyTabletopUseCase } from 'src/domain/tabletop-finder/application/use-cases/fetch-nearby-tabletop'
import { FetchNearbyTabletopController } from './controllers/tabletop-finder/fetch-nearby-tabletop.controller'
import { GetTabletopDetailsController } from './controllers/tabletop-finder/get-tabletop-details.controller'
import { GetTabletopDetailsUseCase } from 'src/domain/tabletop-finder/application/use-cases/get-tabletop-details'
import { FetchPlayerTabletopUseCase } from 'src/domain/tabletop-finder/application/use-cases/fetch-player-tabletop'
import { FetchPlayerTabletopController } from './controllers/tabletop-finder/fetch-player-tabletop.controller'
import { UploadFileController } from './controllers/core/upload/upload-file.controller'
import { UploadFilesAsyncController } from './controllers/core/upload/upload-async-files.controller'
import { UploadFileUseCase } from 'src/domain/core/application/upload/upload-file'
import { UploadFilesAsyncUseCase } from 'src/domain/core/application/upload/upload-files-async'
import { StorageModule } from '../storage/storage.module'
import { EditTabletopUseCase } from 'src/domain/tabletop-finder/application/use-cases/edit-tabletop'
import { EditTabletopController } from './controllers/tabletop-finder/edit-tabletop.controller'
import { GetPlayerDetailsUseCase } from 'src/domain/core/application/use-cases/player/get-player-details'
import { GetPlayerDetailsController } from './controllers/core/player/get-player-details.controller'
import { FetchAllTabletopsController } from './controllers/tabletop-finder/fetch-all-tabletop.controller'
import { FetchAllTabletopsUseCase } from 'src/domain/tabletop-finder/application/use-cases/fetch-all-tabletop'
import { RegisterCredentialsAccountUseCase } from 'src/domain/core/application/use-cases/accounts/register-credentials-account'
import { SendAccountValidationTokenUseCase } from 'src/domain/core/application/use-cases/accounts/send-account-validation-token'
import { EditTabletopLocationController } from './controllers/tabletop-finder/edit-tabletop-location.controller'
import { EditTabletopLocationUseCase } from 'src/domain/tabletop-finder/application/use-cases/edit-tabletop-location'
import { RegisterCredentialsAccountController } from './controllers/core/accounts/register-credentials-account.controller'
import { SendAccountValidationTokenController } from './controllers/core/accounts/send-account-validation-code.controller'
import { ValidateAccountController } from './controllers/core/accounts/validate-account.controller'

@Module({
  imports: [
    forwardRef(() => BullConfigModule),
    DatabaseModule,
    CryptographyModule,
    AuthModule,
    MailModule,
    StorageModule,
  ],
  controllers: [
    // Accounts
    RegisterCredentialsAccountController,
    SendAccountValidationTokenController,
    ValidateAccountController,

    // Sessions
    CredentialsAuthenticateController,

    // Player
    UpdatePlayerController,
    GetPlayerDetailsController,

    // Tabletop
    RegisterTabletopController,
    RegisterTabletopLocationController,
    FetchNearbyTabletopController,
    GetTabletopDetailsController,
    FetchPlayerTabletopController,
    EditTabletopController,
    FetchAllTabletopsController,
    EditTabletopLocationController,

    // Upload\
    UploadFileController,
    UploadFilesAsyncController,
  ],
  providers: [
    // Accounts
    RegisterCredentialsAccountUseCase,
    SendAccountValidationTokenUseCase,
    ValidateAccountUseCase,

    // Sessions
    CredentialsAuthenticateUseCase,

    // Player
    UpdatePlayerUseCase,
    GetPlayerDetailsUseCase,

    // Tabletop
    RegisterTabletopUseCase,
    RegisterTabletopLocationUseCase,
    FetchNearbyTabletopUseCase,
    GetTabletopDetailsUseCase,
    FetchPlayerTabletopUseCase,
    EditTabletopUseCase,
    FetchAllTabletopsUseCase,
    EditTabletopLocationUseCase,

    // Upload\
    UploadFileUseCase,
    UploadFilesAsyncUseCase,
  ],
  exports: [UploadFilesAsyncUseCase],
})
export class HttpModule {}
